...
ws = wb['OneStep']  # 或者使用工作表名称：wb['Sheet1']

# 遍历每一行
for i in range(start_row, start_row + num_rows):
    # 在每一行的指定位置生成随机数
    ws.cell(row=i, column=start_col, value=(i-10)*0.02)  # 到达率
    ws.cell(row=i, column=start_col + 1, value=0.26)  # 服务时间
    ws.cell(row=i, column=start_col + 2, value=3)  # 服务能力，这里假设它是1到10的整数

# 保存工作簿
wb.save('resource/output.xlsx')
